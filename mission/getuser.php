...
$sql="SELECT destino,salidaBodegaFecha,fechaFactura,fechaCronograma FROM facturacion WHERE frt = '".$q."'";
$result = mysqli_query($con, $sql);
$sql2="SELECT diasRetraso,valorMulta,valorPagar FROM facturacion WHERE frt = '".$q."'";
...
$sql3="SELECT SUM(`valorPagar`) as sumatoria, COUNT(*) as registros FROM `facturacion`";
$result3 = mysqli_query($con, $sql3);
if (mysqli_num_rows($result3) > 0) {
    while($row = mysqli_fetch_assoc($result3)) {
		$sum = $row['sumatoria'];
	echo "<center><p>Total valor a pagar: $".$sum." de: ".$row['registros']." registros</p></center>";  
	}
} else {
    echo "0 results";
}
